...
TopNavigationBar::TopNavigationBar()
{
    ...
    // Set learn button to be toggleable
    learnButton.setClickingTogglesState(true);
    
    // Add callback for learn button
    learnButton.onClick = [this]() {
        if (learnButton.getToggleState())
        {
            // Apply immediate tooltip delay
            setTooltipDelay(this, 1);
        }
        else
        {
            // Reset to normal tooltip delay
            setTooltipDelay(this, 800);
        }
        
        // Call the callback to inform MainComponent
        if (learnModeCallback)
            learnModeCallback(learnButton.getToggleState());
    };
    ...
    projectsButton.setTooltip("Projects: Create and manage your plugin projects");
    templatesButton.setTooltip("Templates: Access pre-made plugin templates to get started quickly");
    learnButton.setTooltip("Learn Mode: Toggle to see helpful information about components");
...
void TopNavigationBar::paint(juce::Graphics& g)
{
    ...
    // Special highlight for learn button when toggled on
    if (learnButton.getToggleState()) {
        // Use a more prominent background for active learn mode
        g.setColour(juce::Colours::orange.withAlpha(0.3f));
        g.fillRoundedRectangle(learnButton.getBounds().toFloat().reduced(2.0f), 6.0f);
        
        // Add a border
        g.setColour(juce::Colours::orange);
        g.drawRoundedRectangle(learnButton.getBounds().toFloat().reduced(2.0f), 6.0f, 2.0f);
    } else {
...
// Helper method to recursively update learn mode for all components
void TopNavigationBar::setTooltipDelay(juce::Component* component, int delayMs)
{
